        ...
        <router-link to="/transactionsaproved">
          <div class="report-box zoom-in">
            <div class="box p-5">
              ...
              <div class="text-3xl font-medium leading-8 mt-6">
                <label v-if="aprovedCount">{{
                  formatNumber(aprovedCount)
                }}</label
                ><label v-else>
                  <div class="spinner" style="font-size: 18px"></div>
                </label>
        ...
        <router-link to="/transactionsrevoked">
          <div class="report-box zoom-in">
            <div class="box p-5">
              ...
              <div class="text-3xl font-medium leading-8 mt-6">
                <label v-if="revokedCount">{{
                  formatNumber(revokedCount)
                }}</label
                ><label v-else>
                  <div class="spinner" style="font-size: 18px"></div>
                </label>
  ...
  methods: {
    ...
    async fetchData() {
      try {
        ...
        const response = await axios.get("/api/get/logedmanagertransactions", {
          headers: {
            token: token,
          },
          params: queryParams,
    ...
    async fetchDashboardTotals() {
      try {
        ...
        const response = await axios.get(
          "/api/total/transactionsbystatusfrommanager",
          {
            headers: {
              token: token,
            },
        ...
        if (response.status === 200) {
          ...
          for (const statusType of statusTypes) {
            // Create a sanitized property name for accessing the data
            const propertyName = statusType.toLowerCase().replace(" ", ""); // Convert to lowercase and remove spaces
            this[propertyName] = statusCounts[statusType]?.totalAmount || 0;
            this[propertyName + "Count"] = statusCounts[statusType]?.count || 0;
          }
        } else {
          // Handle non-successful response (e.g., show an error message)
          console.error(
            "Error fetching dashboard totals. Status code:",
            response.status
          );
          // You can also show an error message to the user if needed.

          // Set all values to zero in case of an error
          this.setAllValuesToZero();
        }
      } catch (error) {
    ...
    // Helper function to set all values to zero
    setAllValuesToZero() {
      const statusTypes = [
        "In progress",
        "Revoked",
      ...
      for (const statusType of statusTypes) {
        const propertyName = statusType.toLowerCase().replace(" ", "");
        this[propertyName] = 0;
        this[propertyName + "Count"] = 0;
      }
    },
