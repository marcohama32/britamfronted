<template>
  <div>
    <h2 class="intro-y text-lg font-medium mt-10">My Services</h2>
      ...
      >
        <div class="box">
          <div class="flex items-start px-5 pt-5">
            <div
              v-if="role === 1"
              class="absolute right-0 top-0 mr-5 mt-3 dropdown"
            >
              <a
              ...
              <div class="dropdown-menu w-40">
                <div class="dropdown-content">
                  <router-link :to="`/updateservice/${service._id}`">
                    <label class="dropdown-item"> Edit </label>
                  </router-link>
                </div>
              </div>
            </div>
          ...
          >
            ...
            <div class="w-full flex flex-col lg:flex-row items-left mt-3">
              <div class="">
                <div class="text-slate-500 text-xs mt-0.5">
                  Service price: <strong>{{ formatCurrency(service.servicePrice) }}</strong>
                </div>
                <div class="text-slate-500 text-xs mt-0.5 text-success">
                  Remaining balance:
                  <strong>{{ formatCurrency(service.servicePrice - service.remainingBalance) }}</strong>
                </div>
              </div>
            </div>
          </div>
  ...
  methods: {
    ...
    async fetchData() {
      try {
        ...
        if (this.searchTerm) {
          params.serviceName = this.searchTerm;
        }

        const response = await axios.get("/api/getmyservices", {
          headers: {
            token: token,
          },
        ...
        if (response.data.success) {
          this.myservices = response.data.planService;
          // console.log("Services: ", response.data);
        }

        this.count = response.data.totalServices;
