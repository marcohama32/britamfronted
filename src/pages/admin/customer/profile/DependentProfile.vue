  ...
  <div>
    ...
    <div class="intro-y flex items-center mt-8">
      ...
      <div v-if="role === '1'">
        ...
        <div v-if="CustomerRole === 7">
          <router-link :to="`/updatependentcorporate/${this.$route.params.id}`"
            ><button class="btn btn-secondary mt-5">Edit</button>
          </router-link>
        </div>
        <div v-if="CustomerRole === 8">
          <router-link
            :to="`/updatedependentindividual/${this.$route.params.id}`"
            ><button class="btn btn-secondary mt-5">Edit</button>
          </router-link>
        </div>
      </div>
    ...
    <div class="tab-content mt-5">
      ...
      >
        <div class="grid grid-cols-12 gap-6">
          ...
          <!-- END: Latest Uploads -->
          <!-- BEGIN: Latest Tasks -->
          <div v-if="condition" class="intro-y box col-span-12 lg:col-span-6">
            <div
                ...
                >
                  <div v-if="myMembers.length > 0" class="intro-y">
                    <div
                      ...
                      :key="member._id"
                    >
                      <router-link :to="`/customerprofile/${member._id}`">
                        <div
...
export default {
  ...
  methods: {
    getColumnSpanClass() {
      const userRole = this.CustomerRole;
      if (userRole === 4 || userRole === 5) {
        return "intro-y box col-span-12 lg:col-span-12";
      } else if (userRole === 7 || userRole === 8) {
        return "intro-y box col-span-6 lg:col-span-6";
      } else {
        // Handle other cases if needed
        return "intro-y box lg:col-span-12";
      }
    },
    ...
    getProfile() {
      ...
      axios
        ...
        .then((response) => {
          ...
          // console.log("Manager", response.data.user.accountOwner.manager.firstName);
          this.myMembers = response.data.user.myMembers;

          // this.manager = response.data.user.manager;
          this.managerFirstName =
            response.data.user.accountOwner.manager.firstName;
          this.managerLastName =
            response.data.user.accountOwner.manager.lastName;
