<template>
    <div>
      <div class="intro-y flex items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">Create Admin</h2>
        <div v-if="loading">
    ...
    methods: {
      ...
      async onCreateAgent() {
        ...
        try {
          ...
          const response = await axios.post("/api/user/admin/create", formData, {
            headers: {
              token: token,
          ...
          this.$emit("Admin created successfully");
        } catch (error) {
          if (
            error.response &&
            error.response.data &&
