<template>
  <div>
    <div class="intro-y flex items-center mt-8">
      <h2 class="text-lg font-medium mr-auto">Create Manager</h2>
      <div v-if="loading">
    ...
    <form
      @submit.prevent="onCreateManager"
  ...
  methods: {
    ...
    async onCreateManager() {
      ...
      try {
        ...
        const response = await axios.post("/api/user/manager/create", formData, {
          headers: {
            token: token,
        ...
        this.loading = false;
        this.$router.push("/employermanagement"); // Redirect to other page
        this.isSuccess = true;
        console.log(response);
        this.$emit("Manager created successfully");
      } catch (error) {
        if (
          error.response &&
          error.response.data &&
