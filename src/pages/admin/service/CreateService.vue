<template>
  <div>
    <div class="intro-y flex items-center mt-8">
      <h2 class="text-lg font-medium mr-auto">Create service for {{ planName }} || {{ formatCurrency(planPrice) }}</h2>
      <div v-if="loading">
  ...
  methods: {
    ...
    async onCreateIndivitualCustomer() {
      ...
      try {
        ...
        formData.append("planId", this.$route.params.id);
        this.btnloading = true;
        this.loading = true;

        const response = await axios.post("/api/service/createservice", formData, {
          headers: {
            token: token,
